...
def test_add_api(client):
    ...
    response = client.post('/add', json={'a': 12, 'b': 8})
    
    # Assert that the response status code is 200 (OK)
    assert response.status_code == 200, f"Expected status code 200, got {response.status_code}"
    
    # Assert that the JSON response contains the correct 'result' value
    assert response.json()['result'] == 20, f"Expected result 15, got {response.json()['result']}"
...
def test_subtract_api(client):
    ...
    response = client.post('/subtract', json={'a': 15, 'b': 5})
    
    # Assert that the response status code is 200 (OK)
    assert response.status_code == 200, f"Expected status code 200, got {response.status_code}"
    
    # Assert that the JSON response contains the correct 'result' value
    assert response.json()['result'] == 10, f"Expected result 5, got {response.json()['result']}"
...
def test_multiply_api(client):
    ...
    # Assert that the JSON response contains the correct 'result' value
    assert response.json()['result'] == 25, f"Expected result 50, got {response.json()['result']}"
...
def test_divide_api(client):
    ...
    # Assert that the JSON response contains the correct 'result' value
    assert response.json()['result'] == 4, f"Expected result 5, got {response.json()['result']}"
...
def test_divide_by_zero_api(client):
    ...
    response = client.post('/divide', json={'a': 14, 'b': 0})
